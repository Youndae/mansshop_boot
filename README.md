...
>>> 최종적으로 상품 페이지 또는 장바구니에서 주문 요청 -> 서버에서 데이터 확인 후 order 토큰(Cookie) 생성 및 Redis에 상품 데이터 담고 반환 -> 결제 API 호출 전 결제 상품 데이터와 Redis 데이터 비교 -> 정상인 경우 API 호츨. 비정상인 경우 세션 만료 응답을 보내면서 order 토큰 제거 후 리다이렉트   
>>> 이런 구조로 설계.   
...
>>> 리팩토링 내역 최종 정리.
>>>> 프론트 엔드 구조를 변경.   
>>>> 기존 구조의 경우 컴포넌트 분리는 잘 되어있는 구조였지만, 내부에서 Axios를 호출하고, 로직이 포함되어 있는 구조.   
>>>> 최근에 유튜브를 보다가 axios 호출이 컴포넌트 내부에서 이루어지는 것 보다 외부에서 처리하도록 분리하는게 더 좋다는 말을 듣고 찾아보니 service -> api 구조로 분리를 많이 한다는 것을 알게 됨.   
>>>> service에서는 별 로직이 없더라도 api를 호출하는 인터페이스와 같은 역할을 담당.   
>>>> 마치 Spring Boot에서 Controller가 Repository를 즉시 호출하도록 하는 경우를 최대한 지양하는 것과 같은 느낌.   
>>>> 중복되는 로직이나 컴포넌트 내부에 작성하기에는 가독성이 떨어지거나 복잡한 경우 utils로 분리 또는 Component 내부에서 함수 분리.   
>>>> 상수도 최대한 활용하는 방향으로 개선하고 각 도메인별 상수 파일로 만들거나 공통인 경우 common 하위에 배치.   
>>>> src 하위에 assets를 만들어뒀는데 여기에는 아이콘이나 정적 이미지 파일들을 배치할 계획.   
>>>> 아직까지는 아무것도 배치하지 않았고 추후 Webpack 적용 과정에서 처리할 계획.   
>>>> 그리고 컴포넌트 네이밍도 좀 더 명확하게 할 수 있도록 최대한 개선.

<br />

### 2025/05/26
> 프론트엔드
>> AccessToken 저장 구조 개선   
>> { token: AccessToken, expires: 만료기간 } 구조.   
>> 구조 개선으로 인해 LocalStorage 저장 처리 및 조회 처리 수정   
> 백엔드
